...
params_lidar = {
    ...
    "localIP": "127.0.0.1",
    "localPort": 9094,
    # 데이터 블록의 크기를 설정합니다. 데이터 블록은 라이다에서 수집된 정보의 덩어리
    "Block_SIZE": int(1206),
    #  X, Y, Z 좌표는 라이다의 위치를 나타내며, 여기서는 0 미터로 설정
    "X": 0, # meter
    "Y": 0,
    "Z": 0.6,
    "YAW": 0, # deg
...
params_cam = {
    ...
    "localIP": "127.0.0.1",
    "localPort": 1232,
    # 데이터 블록의 크기를 나타냅니다. 이 값은 데이터를 패킷 단위로 처리하는 데 사용됩니다
...
class detection_net_class():
    def __init__(self, sess, graph, category_index):
        ...
        # graph와 라벨정보를 받아서 ROS2 topic 통신으로 들어온 이미지를 inference(추론) 하고
    ...
    while rclpy.ok():
     ...
     # 로직 12. bounding box 결과 좌표 뽑기
        if len(boxes_detect) != 0:
            ...
            # 바운딩 박스 좌표를 추출합니다.
            boxes_np = boxes_detect[0]

            # 바운딩 박스 좌표를 이미지 비율에 맞게 정수로 변환합니다.
            x = int(boxes_np[1] * iw)
            y = int(boxes_np[0] * ih)
            w = int((boxes_np[3] - boxes_np[1]) * iw)
            h = int((boxes_np[2] - boxes_np[0]) * ih)
            ...
            bbox = np.array([])  # 바운딩 박스가 없는 경우 빈 배열을 반환합니다.
        ...
        for i in range(bbox.shape[0]):
            ...
            # 바운딩 박스 안에 들어가는 라이다 포인트 추출
            points_inside_bbox = xyz_c[(xy_i[:, 0] >= x) & (xy_i[:, 0] <= x + w) & (xy_i[:, 1] >= y) & (xy_i[:, 1] <= y + h)]

            if len(points_inside_bbox) > 0:
                # 라이다 포인트들의 대표값 계산 (여기서는 평균 사용)
                ostate = np.mean(points_inside_bbox, axis=0)
                ostate_list.append(ostate)

        # 대표값이 존재하면 출력
        if len(ostate_list) > 0:
            print("Detected Object Positions:")
            for i, ostate in enumerate(ostate_list):
                print(f"Object {i + 1}: X={ostate[0]:.2f}m, Y={ostate[1]:.2f}m, Z={ostate[2]:.2f}m")
