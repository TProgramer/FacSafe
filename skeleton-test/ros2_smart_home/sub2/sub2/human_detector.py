...
class HumanDetector(Node):
    ...
    def detect_human(self, img_bgr):
        ...
        # rects_temp는 보행자가 감지된 각각의 bounding box (BBox)를 저장하는 리스트입니다.
        # 각 BBox는 (x, y, width, height) 형식의 튜플로 표현됩니다.
        # x: BBox의 왼쪽 상단 꼭지점의 x 좌표
        # y: BBox의 왼쪽 상단 꼭지점의 y 좌표
        # width: BBox의 너비 (x 좌표 간의 거리)
        # height: BBox의 높이 (y 좌표 간의 거리)
        ...
        if len(rects_temp) != 0:
            ...
            for (x,y,w,h) in rects:
                ...
                print("rects.x:{},rects.y:{},rects.w:{},rects.h:{}".format(x,y,w,h))
                xl.append(int(x+(w//2)))
                yl.append(int(y+(h//2)))
                wl.append(int(w))
                hl.append(int(h))
            ...
            if self.able_to_pub:
                ...
                self.bbox_msg.idx_bbox = list(range(len(rects)))
