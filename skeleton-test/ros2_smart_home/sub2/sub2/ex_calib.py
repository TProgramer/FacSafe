...
def rotationMtx(
    yaw, pitch, roll
):  # 회전 행렬(Rotation Matrix) : Euler 각도(yaw, pitch, roll)를 사용, 3D 공간에서 오브젝트를 회전하는 데 사용
    #  roll : x축 주위의 회전
    #  pitch: y축 주위의 회전
    #  yaw  : z축 주위의 회전
...
def transformMTX_lidar2cam(params_lidar, params_cam):
    ...
    lidar_yaw, lidar_pitch, lidar_roll = (
        params_lidar.get("YAW"),
        params_lidar.get("PITCH"),
        params_lidar.get("ROLL"),
    )
    cam_yaw, cam_pitch, cam_roll = (
        params_cam.get("YAW"),
        params_cam.get("PITCH"),
        params_cam.get("ROLL"),
    )

    lidar_pos = {
        "X": params_lidar.get("X"),
        "Y": params_lidar.get("Y"),
        "Z": params_lidar.get("Z"),
    }
    cam_pos = {
        "X": params_cam.get("X"),
        "Y": params_cam.get("Y"),
    ...
    Tmtx = translationMtx(
        -(cam_pos.get("X") - lidar_pos.get("X")),
        -(cam_pos.get("Y") - lidar_pos.get("Y")),
        -(cam_pos.get("Z") - lidar_pos.get("Z")),
    )
    ...
    R_T = np.array(
        [
            [6.12323400e-17, -1.00000000e00, 0.00000000e00, 0.00000000e00],
            [6.12323400e-17, 3.74939946e-33, -1.00000000e00, 4.00000000e-01],
            [1.00000000e00, 6.12323400e-17, 6.12323400e-17, -2.44929360e-17],
...
def project2img_mtx(params_cam):  # 3D 공간의 포인트를 2D 이미지 평면으로 변환하기 위한 투영 행렬을 생성
    ...
    fc_x = params_cam.get("WIDTH") / (2 * tan_fov)
...
def draw_pts_img(img, xi, yi):  # 이미지 점으로 그려내기 xi, yi는 좌표들의 리스트
    ...
    for ctr in zip(xi, yi):  # x좌표와 y좌표를 만들어 냄. zip : 두 리스트에서 동일한 인덱스의 요소들을 짝지어 주는 함수.
        point_np = cv2.circle(
            point_np, ctr, 2, (255, 0, 0), -1
        )  # ctr을 원의 중심으로 하여 point_np이미지에 원을 그림. 세번째인자 2는 반지름.(픽셀단위)
...
class LIDAR2CAMTransform:  # 라이다 포인트들을 라이다 좌표계에서 카메라 좌표계로 변환하고, 이를 이용하여 이미지 프레임 상의 픽셀 좌표로 투영하기.
    def __init__(self, params_cam, params_lidar):
        ...
        self.RT = transformMTX_lidar2cam(
            params_lidar, params_cam
        )  # 라이다 좌표계에서 카메라 좌표계로 변환한 행렬
...
class SensorCalib(Node):
    ...
    def img_callback(self, msg):
        ...
        np_arr = np.frombuffer(
            msg.data, np.uint8
        )  # 이미지 데이터를 NumPy 배열로 변환 / frombuffer : 버퍼에서 NumPy 배열을 만들어내는 함수(이진 데이터를 NumPy 배열로 변환)
